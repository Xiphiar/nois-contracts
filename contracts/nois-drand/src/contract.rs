...
use crate::state::{
    ...
    BOTS, CONFIG, INCENTIVISED_ROUNDS, INCENTIVISED_ROUNDS_MARKER, SUBMISSIONS, SUBMISSIONS_COUNT,
};
...
pub fn execute(
    ...
    match msg {
        ...
        ExecuteMsg::SetIncentivised { round } => execute_set_incentivised(deps, env, info, round),
...
fn execute_set_incentivised(
    ...
    INCENTIVISED_ROUNDS.save(deps.storage, round, &INCENTIVISED_ROUNDS_MARKER)?;
    Ok(Response::default())
}
...
mod tests {
    ...
    fn set_incentivised_works() {
        ...
        let msg = ExecuteMsg::SetIncentivised { round: GOOD_ROUND };
        ...
        let msg = ExecuteMsg::SetIncentivised { round: BAD_ROUND };
        let err = execute(deps.as_mut(), mock_env(), mock_info(GATEWAY, &[]), msg).unwrap_err();
        assert!(matches!(
            err,
            ContractError::RoundTooLow {
